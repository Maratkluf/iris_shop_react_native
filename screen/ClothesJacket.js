...
export default function ClothesJacket(props) {

  const [Clothes, setClothes] = useState([])

  useEffect(() => {
    firebase.db.collection('Clothes').where('product', '==', 'Куртка').onSnapshot(querySnapshot => {
      const Clothes = [];
      
      querySnapshot.docs.forEach(doc => {
        const {product, brand, name, information, image, price, image2, image3, image4, color, group} = doc.data();
        Clothes.push({
          id: doc.id,
          product, 
          brand, 
  ...
  sortAsc = () => {   
    firebase.db.collection('Clothes').orderBy('price', 'asc').where('product', '==', 'Куртка').onSnapshot(querySnapshot => {
      const Clothes = [];
  ...
  sortDesc = () => {   
      firebase.db.collection('Clothes').orderBy('price', 'desc').where('product', '==', 'Куртка').onSnapshot(querySnapshot => {
        const Clothes = [];
